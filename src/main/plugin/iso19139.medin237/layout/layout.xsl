...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="iso19139.medin237schema" select="/root/gui/schemas/iso19139.medin237"/>
  <xsl:variable name="iso19139.medin237labels" select="$iso19139.medin237schema/labels"/>
  <xsl:variable name="iso19139.medin237codelists" select="$iso19139.medin237schema/codelists"/>
  <xsl:variable name="iso19139.medin237strings" select="$iso19139.medin237schema/strings"/>

  <!-- Match codelist values. Must use iso19139.medin237 because some 
	     19139 codelists are extended in gemini22 - if the codelist exists in
			 iso19139.medin237 then use that otherwise use iso19139 codelists 
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="30000" match="*[*/@codeList and $schema='iso19139.medin237' and name()!='gmd:dateType']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.medin237codelists" required="no"/>
    ...
    <xsl:variable name="elementName" select="name()"/>
		...
		<!-- check iso19139.medin237 first, then fall back to iso19139 -->
	...
	<!--
    Take care of enumerations. Same as for codelists, check iso19139.medin237
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="30000" match="*[gn:element/gn:text and $schema='iso19139.medin237']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.medin237codelists" required="no"/>
  ...
  -->
  <xsl:template mode="mode-iso19139" priority="2000" match="gmd:CI_Date/gmd:date[$schema='iso19139.medin237']">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    <xsl:param name="codelists" select="$iso19139.medin237codelists" required="no"/>
